...
阶段 1 先不接复杂 AI，不是为了省事，而是因为售后这条链路本身就有金额、状态、审批、退款、审计这些强约束。它们如果不先定住，后面再加 AI，只会把不稳定放大。

这个阶段先把业务骨架搭起来，目的是让系统先具备三种能力：

- 能判断谁的订单、能不能处理，避免越权和伪造金额。
- 能把每一次状态变化记录下来，知道为什么从一个状态走到另一个状态。
- 能让重复请求收敛到同一个结果，避免重复退款和脏数据。

## 这样做的好处

1. 先把状态机和幂等做稳，后面接 AI、风控和 RAG 时不用推倒重来。
2. 把审批任务单独拆出来，人工节点更清楚，权限控制也更容易收口。
3. 订单快照优先于前端传值，能避免金额和归属被前端伪造。
4. 售后单、工单、流程、审批、退款、审计分开后，每一层的职责都更单一，出问题时更容易定位。
