...
> BookMall AI After-Sales Operations Platform
>
> 本文档描述 BookMall 完成企业级 AI 售后改造后的目标形态。当前仓库仍是图书商城基线，后续按 `docs/阶段实施方案.md` 分阶段落地到 README 所示状态。

## 项目定位

这不是一个“AI 客服聊天框”，而是一套把电商售后业务放进系统里的企业级 AI 运营平台：
...
项目沿用 BookMall 已有的用户、图书、购物车、库存、订单、支付、网关等交易能力，向上新增售后域、流程编排、规则版本、审批、审计、AI 编排、MCP 领域工具和售后规则知识库。
...
第一周不追求完整企业级，优先用 Mock LLM、Mock 物流、MySQL + Redis 跑通闭环；后续一个月替换为真实 LLM、向量库、监控和压测。
...
| `bookmall-book` | 8070 | 商品信息 |
| `bookmall-cart` | 8083 | 购物车交易链路保留 |
